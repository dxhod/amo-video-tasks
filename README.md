...
**Статус:** основний цикл працює локально; unit/FFmpeg, Supabase integration, Auth і командний E2E пройдено. Демонстраційний проєкт має два окремі рендери. Зовнішні GitHub/Supabase/Railway-ресурси ще не створено; live URL та реальний Telegram smoke-test відсутні. Фактичні результати й залишкові перевірки — [TEST_RESULTS](docs/TEST_RESULTS.md).
...
Для повторного монтажу поверніть задачу в «У роботі» та оберіть потрібну версію. Після зміни її попередній результат позначається як застарілий; потрібен новий рендер, щоб передати актуальну версію на перевірку. Номер версії зберігається. «Нова версія з цієї» створює окрему копію; редагування старої версії не змінює вибір актуальної версії задачі.
...
На контрольній точці 2026-09-07 production web у Docker доступний на `http://127.0.0.1:3100`; демо-акаунти ті самі. Контейнерний E2E із двома MP4 та review пройшов. Це локальна перевірка, публічний Railway URL ще не створено.
...
- GitHub: ще не опубліковано.
- Railway live URL: ще не видано.
