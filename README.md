...
**Live demo:** https://amo-web-production.up.railway.app — Railway Free (web + worker), окремий Supabase Free. Репозиторій: https://github.com/dxhod/amo-video-tasks, поточний live-коміт `2576a5f`. Railway web/worker успішно розгорнуті, web healthcheck повертає HTTP 200. Cloud analysis, незалежні MP4, монтаж і review двома користувачами, Gmail-підтвердження та відновлення пароля перевірені. 2026-09-08 також перевірено 1080p/4K на Railway і Telegram-алерт для пошкодженого відео через [@AMO_apps_test_bot](https://t.me/AMO_apps_test_bot). Дані входу для власника — `.local/live-demo.json` (gitignored); локальні акаунти з `.local/demo.json` окремі. Фактичні результати й залишкові перевірки — [TEST_RESULTS](docs/TEST_RESULTS.md).
...
- Queue lease, heartbeat і fencing спроб захищають від повторного виконання та рестарту worker. Унікальний шлях кожної спроби зберігає результати попередніх версій. Той самий worker незалежним циклом забирає повідомлення з `notification_outbox` і доставляє їх у Telegram.

Докладніше: [рішення й обмеження](docs/DECISIONS.md), [навігація проєктом](PROJECT_MAP.md).

## Локальний запуск

Потрібні Node.js 22.12+, npm, запущений Docker Desktop і FFmpeg/ffprobe у PATH. Docker-образи та CI використовують Node.js 24.13.1. Версії npm-залежностей закріплені lockfile.
...
| На review передано попередню готову версію | Видимі лише «Оригінал» і вибрана версія; у done — лише прийнята | Supabase integration та browser E2E |
...
- [Рішення й виправлення під час роботи з AI](docs/AI_WORK_LOG.md) та [відбір ключових фрагментів реальної AI-сесії](docs/AI_SESSION_EXCERPT.md).
